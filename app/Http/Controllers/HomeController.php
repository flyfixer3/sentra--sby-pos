...
class HomeController extends Controller
{
    ...
    public function index() {
        ...
        $purchase_returns = Purchase::completed()->sum('total_amount');
        $product_costs = 0;

        foreach (Sale::completed()->with('saleDetails')->get() as $sale) {
            foreach ($sale->saleDetails??[] as $saleDetail) {
                $product_costs += ((float) $saleDetail->product_cost * (int) $saleDetail->quantity);
            }
        }

        $revenue = ($sales - $sale_returns) / 1;
        $profit = $revenue - $product_costs;

        return view('home', [
            'revenue'          => $revenue,
            'sale_returns'     => $sale_returns / 1,
            'purchase_returns' => $purchase_returns / 1,
            'profit'           => $profit
        ]);
    }
