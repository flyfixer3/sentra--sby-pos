...
class StocksDataTable extends DataTable
{
    ...
    public function query(Stock $model): Builder
    {
        ...
        if ($isAllBranchMode) {
            $q->select([
                ...
                DB::raw('SUM(stocks.qty_available) as qty_available_sum'),
                DB::raw('COALESCE(defects.defect_qty, 0) as defect_qty'),
                DB::raw('COALESCE(damaged.damaged_qty, 0) as damaged_qty'),
                DB::raw('GREATEST(SUM(stocks.qty_available) - COALESCE(defects.defect_qty,0), 0) as good_qty'),
            ])
    ...
    public function html()
    {
        ...
        return $this->builder()
            ...
            ->parameters([
                ...
                'searching' => false, // ✅ hilangin search bar DataTables
                'language' => [
    ...
    protected function getColumns(bool $isAllBranchMode): array
    {
        $cols = [
            ...
            ['data' => 'good_qty', 'name' => 'good_qty', 'title' => 'Good (Sellable)', 'class' => 'text-end'],
        ...
        if (!$isAllBranchMode) {
            $cols[] = ['data' => 'action', 'name' => 'action', 'title' => 'Detail', 'orderable' => false, 'searchable' => false, 'class' => 'text-center'];
        }
