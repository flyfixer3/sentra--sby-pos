        ...
        <div class="card">
            <div class="pd-header">
                <div>
                    ...
                    <p class="pd-sub mb-0">
                        Based on PO: <span class="pd-badge"><i class="bi bi-receipt"></i> {{ $purchaseOrder->reference }}</span>
                    </p>
                </div>
            ...
            <div class="card-body">
                ...
                <div class="row">
                    ...
                    {{-- ✅ FIX: bukan shipping supplier address, tapi receiving address (alamat cabang/warehouse kita) --}}
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="warehouse_id" class="form-control" required>
                            @foreach($warehouses as $wh)
                                <option value="{{ $wh->id }}"
                                    {{ (int) old('warehouse_id', $defaultWarehouseId ?? 0) === (int) $wh->id ? 'selected' : '' }}>
                ...
                {{-- Product Table --}}
                @php
                    $remainingDetails = $purchaseOrder->purchaseOrderDetails
                        ->filter(fn($detail) => ($detail->quantity - $detail->fulfilled_quantity) > 0);
                @endphp

                <div class="d-flex align-items-center justify-content-between mt-4">
                    <h5 class="mb-0">Items to Receive</h5>

                    {{-- ✅ tombol umum, keliatan --}}
                ...
                <div class="table-responsive mt-2">
                    <table class="table table-striped pd-table">
                        ...
                        <tbody>
                            @forelse($remainingDetails as $detail)
                                @php
                                    $maxQty = (int) ($detail->quantity - $detail->fulfilled_quantity);
                                @endphp
                                ...
                                <tr data-row="{{ $detail->id }}">
                                    ...
                                    <td>
                                        <div class="pd-qty-wrap">
                                            <input type="number"
                                                   ...
                                                   data-max="{{ $maxQty }}">
                                            <div class="pd-mini">
                                                Remaining: <span class="remaining-qty" id="remaining-qty-{{ $detail->id }}">{{ max(0, $maxQty - (int) old('quantity.'.$detail->id, $maxQty)) }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        {{-- ✅ tombol apply jelas di kolom action --}}
                                        <button type="button"
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // realtime update remaining
    qtyInputs.forEach((input) => {
        input.addEventListener("input", () => updateRemaining(input));
        updateRemaining(input);
    });

    // Apply row (UX: highlight row so user feels "action happened")
